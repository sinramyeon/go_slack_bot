...
func (h interactionHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case actionSelect:
		...
		originalMessage.Attachments[0].Text = fmt.Sprintf("%s 할까요??", strings.Title(value))
	...
	case actionStart:
		...
		return
	case actionCancel:
		title := fmt.Sprintf(":x: @%s 님께서 취소해 버리셨습니다.", message.User.Name)
		responseMessage(w, message.OriginalMessage, title, "")
		return
	default:
		log.Printf("[ERROR] %s 실행 중 에러가 발생했습니다.", action.Name)
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
}
